...
<template>
  <div class="min-h-screen bg-[#0B1220]">
    <TopNav title="AI Novel Studio" right-label="刷新列表" :right-loading="isLoading" @right-click="refresh" />

    <div class="mx-auto max-w-6xl px-4 py-6">
      <div class="mb-4 flex items-end justify-between gap-4">
        <div>
          <div class="text-base font-semibold text-zinc-100">小说列表</div>
          <div class="mt-1 text-xs text-zinc-400">展示后端返回的全部小说</div>
        </div>
        <div class="flex items-center gap-3">
          ...
          <div class="text-xs text-zinc-400">共 {{ items.length }} 本</div>
        </div>
      </div>
      ...
      <div v-if="isLoading" class="space-y-3">
        <div v-for="i in 5" :key="i" class="animate-pulse rounded-lg border border-zinc-800/60 bg-[#111A2E] p-4">
      ...
      <div v-else-if="items.length === 0" class="rounded-lg border border-zinc-800/60 bg-[#111A2E] p-6">
        <div class="text-sm font-semibold text-zinc-100">暂无小说</div>
        <div class="mt-1 text-xs text-zinc-400">当前数据库里还没有小说记录。</div>
      </div>

      <div v-else class="space-y-3">
        <NovelListItem v-for="n in items" :key="n.id" :item="n" />
      </div>
    </div>
  </div>
</template>
